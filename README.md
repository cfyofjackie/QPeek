...
这是一个以学习为目的的项目，灵感来自 macOS Quick Look。当前首个 SLC（最小完整功能切片）已经完成，核心预览流程曾在两台 Windows 11 电脑上通过手动测试。
...
- TXT 使用可滚动的只读文本区域；Markdown 会渲染标题、列表、任务列表、引用、链接和代码等常用样式。
...
- 只有 QPeek 或对应 Explorer 处于相关前台状态时才处理预览快捷键，不影响其他软件中的输入。
- 拖动预览窗口后会记住其中心位置，下次预览继续使用该位置。
- 程序只允许一个实例运行，并提供系统托盘图标和退出菜单。

## 下载与使用

请前往项目的 [GitHub Releases 页面](https://github.com/cfyofjackie/QPeek/releases)，在 `QPeek v0.2.0` 下展开 **Assets**，然后点击 **QPeek-v0.2.0-win-x64.zip** 下载。
...
如果 Windows SmartScreen 提示未知发布者，请只在确认文件来自本项目 GitHub Release 时继续运行。目前版本没有代码签名。
...
- [RELEASE_NOTES.md](RELEASE_NOTES.md)：当前 GitHub Release 可直接使用的版本说明。
